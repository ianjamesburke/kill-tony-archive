"""
Test: 2-second window laughter detection on 60 minutes of ep 738.
"""
import json
import os
import sys
...
"w" = window index (0 = seconds 0-1, 1 = seconds 2-3, 2 = seconds 4-5, etc.)
...
def main():
    ...
    print(f"2-second windows: {num_windows} windows for {DURATION}s")
    print(f"Uploading...")
    ...
    # Heatmap: each char = 2 seconds, each row = 1 minute (30 chars per row)
    chars = {0: "·", 1: "░", 2: "▒", 3: "▓", 4: "█"}
    print(f"\nHeatmap (each char = 2 seconds, each row = 1 minute):")
    print(f"       {'0':>3}{'':>5}{'10':>3}{'':>5}{'20':>3}{'':>5}{'30':>3}{'':>5}{'40':>3}{'':>5}{'50':>3}")
    for m in range(DURATION // 60):
        start = m * 60
        # One char per 2-second window
        bar = ""
        for w in range(30):  # 30 windows of 2s = 60s
            sec = start + w * 2
            if sec < DURATION:
                bar += chars.get(timeline[sec], "?")
            else:
                bar += " "
        print(f"  {m:2d}:00 |{bar}|")
    ...
    print(f"\n  Active: {active}s ({active/DURATION*100:.1f}%) | Silent: {DURATION-active}s")
    ...
    out = Path(__file__).parent / "test_results_60min_2sec.json"
