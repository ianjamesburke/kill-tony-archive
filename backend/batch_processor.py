...
PASS1_MODEL = "gemini-3.1-flash-lite"  # legacy audio transcription (fallback if Modal unavailable)
...
PIPELINE_VERSION = 4  # v4: WhisperX on Modal GPU replaces Gemini for Pass 1
USE_WHISPERX = True  # use Modal WhisperX GPU for Pass 1 transcription
CHUNK_MINUTES = 20
OVERLAP_MINUTES = 3
...
WHISPERX_PASS2_PROMPT = """
You are analyzing a Kill Tony episode transcript. The transcript was generated by WhisperX (automatic speech recognition) and does NOT have speaker labels. You must identify speakers from context.
This is episode #{episode_number} of Kill Tony.
{chunk_context}
SPEAKER IDENTIFICATION GUIDE:
- Tony Hinchcliffe is the host. He introduces comedians, pulls names from the bucket, gives feedback after sets.
- Brian Redban is the co-host. He opens the show ("Hey this is Redban..."), sometimes gives number ratings, mentions "secret show."
- Guest judges sit on the panel. Tony introduces them at the start.
- Comedians perform 1-minute sets. Tony announces their name before each set.
- The band plays walk-up music between sets.

SHOW FLOW:
1. Redban opens -> Tony takes over -> introduces guests
2. For each set: Tony pulls a name -> band plays walk-up -> comedian performs ~1 min -> Tony/guests/Redban give feedback
3. Regular performers (Hans Kim, William Montgomery, David Lucas, Uncle Laser, Kam Patterson, etc.) get 2-3 min sets

SET BOUNDARY SIGNALS — a new set begins when you see any of these phrases followed by a name:
- "Make some noise for your next comedian"
- "Give it up for" / "Let's hear it for" / "Let's give it up for"
- "Your next comedian" / "Our next comedian"
- "Come on up" (followed by a name being called)
- Tony reads a name off a slip of paper and says "let's meet them together"
After the intro + band walk-up music, the NEXT speech is the comedian's set.

CRITICAL — SET vs INTERVIEW:
A 1-minute set ends when the buzzer sounds, the band plays the comedian off, or the comedian says "that's my time."
What follows is the POST-SET INTERVIEW: Tony and the panel talking WITH the comedian about their life, jokes, background.
The interview is NOT a set. Do NOT extract it as a new comedian's set.
...
FIELD RULES:
- venue: Use ONLY one of these known venue names (pick the closest match):
  "Comedy Mothership" (the home venue in Austin),
  "Moody Center" (arena shows in Austin),
  "Vulcan Gas Company" (previous Austin venue, eps ~300-500s),
  "Antone's Nightclub" (brief Austin venue before Vulcan),
  "The Comedy Store" (original LA venue, early episodes).
...
- status: THREE options: "regular", "bucket_pull", or "special_request".
  KNOWN REGULARS (always mark as "regular"): Hans Kim, William Montgomery, David Lucas, Uncle Laser, Kam Patterson, Michael Lair, Ellis Aych, Aron Rhodes, Carlos Suarez.
  Also mark status = "regular" if Tony introduces them as a regular, says "brand new set from", their set runs >1 min, they close the show, or they are introduced as a returning golden ticket winner.
  Mark status = "special_request" when someone is brought up outside the normal bucket draw or regular rotation.
...
- set_transcript: copy their EXACT words during the performance only
TRANSCRIPT (timestamps are accurate to the second):
{transcript}
"""
...
def pass1_whisperx(audio_path: Path) -> list[dict]:
    """Transcribe episode audio using WhisperX on Modal GPU.

    Sends the full audio file to a T4 GPU, returns transcript entries
    in the same format pass2_analyze() expects.
    """
    from modal_whisperx import app, transcribe

    log.info(f"PASS 1 (WhisperX): Sending {audio_path.stat().st_size / 1024 / 1024:.1f} MB to Modal GPU...")
    ...
    log.info(f"  WhisperX done in {wall_time:.1f}s (GPU: {timings['total_s']:.1f}s)")
    log.info(f"  {len(segments)} segments, {total_words} words")

    # Convert WhisperX segments to Pass 1 entry format for compatibility
    entries = []
    for seg in segments:
    ...
    log.info(f"  Converted to {len(entries)} transcript entries")
    return entries
...
def _pass2_call(client: genai.Client, transcript_text: str, episode_number: int, has_speakers: bool, chunk_context: str = "") -> dict:
    """Single Pass 2 API call. Returns parsed JSON dict."""
    if has_speakers:
        prompt = PASS2_PROMPT.format(transcript=transcript_text, episode_number=episode_number)
    else:
        prompt = WHISPERX_PASS2_PROMPT.format(transcript=transcript_text, episode_number=episode_number, chunk_context=chunk_context)
...
def pass2_analyze(client: genai.Client, transcript: list[dict], episode_number: int) -> dict:
    ...
    has_speakers = any(e.get("speaker", "unknown") != "unknown" for e in transcript)
    ...
    def format_lines(entries: list[dict]) -> str:
        ...
        for entry in entries:
            ...
            if has_speakers:
                ...
                lines.append(f"[{int(ts)}s / {int(ts//60):02d}:{int(ts%60):02d}] {text}")
        return "\n".join(lines)
    ...
    for idx, (start, end, segs) in enumerate(chunks):
        chunk_end_actual = min(end, int(last_ts))
        log.info(f"  Pass 2 chunk {idx+1}/{len(chunks)} ({start//60:.0f}–{chunk_end_actual//60:.0f}min, {len(segs)} segs)")
        chunk_context = (
            f"You are analyzing chunk {idx+1} of {len(chunks)} for this episode "
            f"(time range {start//60:.0f}–{chunk_end_actual//60:.0f} minutes).\n"
        )
        data = _pass2_call(client, format_lines(segs), episode_number, has_speakers, chunk_context)
...
def save_episode(yt_info: dict, transcript: list[dict], analysis: dict, guests: list[str]):
    ...
    backfill_comedian_ages(episode_number)


def backfill_comedian_ages(episode_number: int):
    """Fill missing disclosed_age for comedians in this episode using their age from prior appearances.

    Uses the youngest known age as baseline (hallucinated ages tend to be too high).
    Estimates birthday as 6 months before the reference episode date, then calculates
    age at each appearance based on elapsed time.
    """
    with sqlite3.connect(DB_PATH) as conn:
        conn.row_factory = sqlite3.Row
        comedians = conn.execute(
            "SELECT DISTINCT comedian_name FROM sets WHERE episode_number = ? AND disclosed_age IS NULL",
            (episode_number,),
        ).fetchall()
        ...
        updated = 0
        for row in comedians:
            name = row["comedian_name"]
            ref = conn.execute(
                """SELECT s.disclosed_age, e.date
                   FROM sets s JOIN episodes e ON s.episode_number = e.episode_number
                   WHERE s.comedian_name = ? AND s.disclosed_age IS NOT NULL
                   ORDER BY s.disclosed_age ASC LIMIT 1""",
                (name,),
            ).fetchone()
            ...
            ref_age = ref["disclosed_age"]
            ref_date = datetime.strptime(ref["date"], "%Y%m%d")
            est_birth_year = ref_date.year - ref_age
            est_birthday = datetime(est_birth_year, ref_date.month, ref_date.day) - timedelta(days=182)

            nulls = conn.execute(
                """SELECT s.set_id, e.date
                   FROM sets s JOIN episodes e ON s.episode_number = e.episode_number
                   WHERE s.comedian_name = ? AND s.disclosed_age IS NULL""",
                (name,),
            ).fetchall()

            for n in nulls:
                ep_date = datetime.strptime(n["date"], "%Y%m%d")
                age_at_ep = int((ep_date - est_birthday).days / 365.25)
                conn.execute("UPDATE sets SET disclosed_age = ? WHERE set_id = ?", (age_at_ep, n["set_id"]))
                updated += 1

        if updated:
            conn.commit()
            log.info(f"Backfilled {updated} missing ages for episode #{episode_number}")
...
def process_episode(client: genai.Client, ep: dict, model_override: str | None = None):
    ...
    # Pass 1: Transcribe
    if USE_WHISPERX:
        transcript = pass1_whisperx(audio_path)
    else:
